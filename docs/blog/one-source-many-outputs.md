...
Today that source is hand-written JSON and YAML — comfortable for a technical author or an automated pipeline, less so for a classroom teacher. A **visual authoring UI is planned**: a teacher-facing editor over this same canonical source, so a lesson can be built without touching JSON by hand. It changes nothing about the model — the source stays the contract, and the editor becomes one more tool that reads and writes it.
