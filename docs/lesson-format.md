...
| `W-CLOZE-NO-CARRIER` | A `cloze` sentence carries nothing but its blanks (only markers, whitespace and punctuation remain once the `___` are removed), so there is no gap text to read around: the question sits in the prompt and the blank teaches nothing. The exercise is a question with an answer, and a native type says that directly - `multiple_choice` in `select` mode, `free_text` in `type` mode. Both exist (`multiple_choice` since schema v1.6), and a consumer renders them as one question with its options instead of a gap. A warning, not an error: the shape is valid and can be deliberate. `multiselect` is exempt by design - its `sentence` IS the question and carries no markers at all. |
...
| `W-HINT-LENGTH` | A hint reveals the answer length (e.g. "four letters"). Consumers that display an answer-length indicator make such a hint redundant; on other consumers it gives part of the answer away. Checked on the exercise `hint` and on every `blanks[].hint` (each at its own path), not on card hints, where a character count can be teaching content. The rule looks for a count directly before a length noun: digits, German and English number words up to twelve, the article forms (`mit einem Buchstaben`) and the single-character adjectives (`ein einzelnes Zeichen`, `a single character`), before `Buchstabe(n)`, `Zeichen`, `letter(s)` or `character(s)`; also the reversed form with a colon (`Buchstaben: 4`) and the `-buchstabig` adjectives. Word boundaries are Unicode-aware, so word parts and compounds stay silent (`Achte`, `bestimmten`, `Leerzeichen`, `Fragezeichen`, `characteristic`). One rule instead of two: the content template carried its own stricter copy (engine#186). |
