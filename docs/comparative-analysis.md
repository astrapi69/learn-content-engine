...
**Benchmark Study for the `learn-content-engine` (package 0.31.0, lesson schema currently 1.17)**
...
This document presents a systematic comparative analysis of exercise types across established e-learning platforms (Moodle, H5P, QTI 3.0, Canvas LMS, Duolingo) in the context of the `learn-content-engine`. Two version numbers matter and are easy to conflate: the npm package is at **0.31.0**, the lesson schema it validates against is currently **1.17** (`x-schema-version` in `schema/lesson.schema.json`). This document benchmarks the schema's exercise types.
...
| **Exercise Type Category** | **Moodle** | **H5P** | **QTI 3.0** | **Duolingo** | **learn-content-engine (schema currently 1.17)** |
|----------------------------|------------|---------|-------------|--------------|----------------------------------|
...
The core types (schema currently 1.17) primarily cover the lower levels:
...
The `learn-content-engine` (package 0.31.0, schema currently 1.17) is in an **excellent strategic position**:
...
Runtime footprint of `learn-content-engine`: three dependencies (`yaml`; `ajv` only for the structural layer, the `/rules` subpath runs without it; `@rgrove/parse-xml` only behind the `/qti` subpath), about 1,500 tests (1,533 on 2026-09-25), no framework. Stars measure attention, not quality, but they are the honest proxy for the size of the community that will answer a question or fix a bug.
